#!/bin/sh
#
# Подготовка перед первым запуском:
#  - генерирует ./init/initdb.sql (схема БД Guacamole)
#  - создаёт папку ./record для записей сессий
#
# TLS-сертификаты генерировать не нужно: Caddy получает их
# автоматически через Let's Encrypt при первом запуске.

# проверяем, что docker запущен
if ! (docker ps >/dev/null 2>&1)
then
	echo "docker daemon not running, will exit here!"
	exit
fi

echo "Preparing folder init and creating ./init/initdb.sql"
mkdir ./init >/dev/null 2>&1
chmod -R +x ./init
docker run --rm 'guacamole/guacamole:1.6.0' /opt/guacamole/bin/initdb.sh --postgresql > ./init/initdb.sql
echo "done"

echo "Preparing folder record and set permissions"
mkdir ./record >/dev/null 2>&1
chmod -R 777 ./record
echo "done"
